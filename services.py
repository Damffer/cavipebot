...
def administrar_chatbot(text,number, messageId, name, timestamp):
    ...
    db_type = 'mssql' # previamente configuramos solo mysql, postgresql, dynamodb
    ...
    cursor = conn.cursor()
    query = f"SELECT nombre FROM empleados WHERE telefono = '{number}'"  # Suponiendo que la tabla se llama "usuarios"
    cursor.execute(query)
    result = cursor.fetchone()
    cursor.close()

    if result:
        # Si encontramos el nombre, lo asignamos
        user_name = result[0]
    else:
        # Si no encontramos el nombre, usamos un nombre predeterminado
        user_name = ""

    # Personalizamos el mensaje
    if "hola" in text:
        body = f"¡Hola {user_name}! 👋 Bienvenido a Cavipetrol ¿Cómo podemos ayudarte hoy?"
        footer = "Cavipetrol"
        options = ["🎫 Generar solicitud", "🔍 Ver Estado Solicitud"]
        ...
        db_manager.create_ticket(db_type, conn, cedula, monto, plazo, tipo_solicitud)  
