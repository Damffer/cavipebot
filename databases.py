...
class DatabaseManager:
    def __init__(self):
        self.databases = [
            ...
            {'type': 'dynamodb', 'connection_str': {'region_name':'us-east-1', 'aws_access_key_id':'AKIAYKD2PK23BETDJ6GZ', 'aws_secret_access_key':'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'}},
             {'type': 'mssql', 'connection_str': 'DRIVER={ODBC Driver 17 for SQL Server};SERVER=127.0.0.1,1433;DATABASE=cavipetrol;UID=sa;PWD=C4vip3tr0l'}
        ]
    ...
    def connect(self, db_type):
        ...
        if db_type == 'postgresql':
            ...
            return pyodbc.connect(db['connection_str'])
    ...
    def create_ticket(self, db_type, conn, cedula, monto, plazo, tipo_solicitud):
        if db_type == 'postgresql' or db_type == 'mysql' or db_type == 'mssql':
            cur = conn.cursor()

                
            query = f"INSERT INTO SOLICITUDES (CEDULA, MONTO, PLAZO, TIPO_SOLICITUD) VALUES ('{cedula}', '{monto}', '{plazo}', '{tipo_solicitud}')"
            conn.commit()
            cur.close()
        # elif db_type == 'dynamodb':
    ...
    def get_offer(self, db_type, conn, ticket_id):
        if db_type == 'postgresql' or db_type == 'mysql' or db_type == 'mssql':
            cur = conn.cursor()
